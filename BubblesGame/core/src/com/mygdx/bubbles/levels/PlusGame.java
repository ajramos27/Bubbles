...
public class PlusGame extends AbstractScreen {
	
	private int num=2, n=0, nbad=2;
	public float counter=60, timeCounter;
	private ActorBubble[] burbujas = new ActorBubble[500];
	private ActorBad[] bads= new ActorBad[3];
	private ActorBad bbad= new ActorBad();
	...
	public PlusGame(final Bubbles game) {
		super(game);		
	...
	public void show(){
		...
		stage.addActor(bbad);
			...
			bbad.addCaptureListener(new InputListener() {
					@Override
					public boolean touchDown(InputEvent event, float x, float y,
							int pointer, int button) {
						game.setScreen(new GameOver1(game, n, v));
